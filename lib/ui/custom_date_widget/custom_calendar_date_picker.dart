...
import 'package:rti_assignment/ui/design_system/asset_name.dart';
...
class _CustomCalendarDatePickerState extends State<CustomCalendarDatePicker> {
  ...
  void didUpdateWidget(CustomCalendarDatePicker oldWidget) {
    ...
    if (!CustomDateUtils.isSameDay(widget.initialDate, oldWidget.initialDate)) {
      _currentDisplayedMonthDate =
          DateTime(widget.initialDate.year, widget.initialDate.month);
      _selectedDate = widget.initialDate;
    }
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  void initState() {
    ...
    _pageController = PageController(
        initialPage:
            CustomDateUtils.monthDelta(widget.firstDate, _currentMonth));
  ...
  void _handleMonthPageChanged(int monthPage) {
    setState(() {
      final DateTime monthDate =
          CustomDateUtils.addMonthsToMonthDate(widget.firstDate, monthPage);
      if (!CustomDateUtils.isSameMonth(_currentMonth, monthDate)) {
        _currentMonth = DateTime(monthDate.year, monthDate.month);
        widget.onDisplayedMonthChanged(_currentMonth);
        if (_focusedDay != null &&
            !CustomDateUtils.isSameMonth(_focusedDay, _currentMonth)) {
  ...
  DateTime? _focusableDayForMonth(DateTime month, int preferredDay) {
    final int daysInMonth =
        CustomDateUtils.getDaysInMonth(month.year, month.month);
  ...
  void _showMonth(DateTime month, {bool jump = false}) {
    final int monthPage = CustomDateUtils.monthDelta(widget.firstDate, month);
    if (jump) {
  ...
  void _handleGridFocusChange(bool focused) {
    setState(() {
      if (focused && _focusedDay == null) {
        if (CustomDateUtils.isSameMonth(widget.selectedDate, _currentMonth)) {
          _focusedDay = widget.selectedDate;
        } else if (CustomDateUtils.isSameMonth(
  ...
  void _handleDirectionFocus(DirectionalFocusIntent intent) {
    ...
    setState(() {
      ...
      if (nextDate != null) {
        ...
        if (!CustomDateUtils.isSameMonth(_focusedDay, _currentMonth)) {
  ...
  DateTime? _nextDateInDirection(DateTime date, TraversalDirection direction) {
    ...
    DateTime nextDate = CustomDateUtils.addDaysToDate(
    ...
    while (!nextDate.isBefore(widget.firstDate) &&
      ...
      nextDate = CustomDateUtils.addDaysToDate(
  ...
  Widget _buildItems(BuildContext context, int index) {
    final DateTime month =
        CustomDateUtils.addMonthsToMonthDate(widget.firstDate, index);
    return _DayPicker(
        ...
        children: <Widget>[
          Container(
            ...
            child: Row(
              ...
              children: <Widget>[
                IconButton(
                  icon: Image.asset(Assets.leftImage),
                  color: controlColor,
                ...
                IconButton(
                  icon: Image.asset(Assets.rightImage),
                  color: controlColor,
            ...
            child: FocusableActionDetector(
              ...
              child: _FocusedDate(
                ...
                child: PageView.builder(
                  ...
                  itemCount: CustomDateUtils.monthDelta(
...
class _DayPickerState extends State<_DayPicker> {
  ...
  List<Widget> _dayHeaders(
    ...
    for (int i = localizations.firstDayOfWeekIndex; true; i = (i + 1) % 7) {
      final String weekday = strDays[i];
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle headerStyle = RTITextStyles.dialogWeekNameStyle;
    final TextStyle dayStyle = TextStyle(
        fontSize: 15, fontWeight: FontWeight.w400, color: RTIColors.liteBlack);
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        final bool isSelectedDay =
            CustomDateUtils.isSameDay(widget.selectedDate, dayToBuild);
        final bool isToday =
            CustomDateUtils.isSameDay(widget.currentDate, dayToBuild);
...
class YearPicker extends StatefulWidget {
  ...
  YearPicker({
    ...
    this.dragStartBehavior = DragStartBehavior.start,
        ...
        assert(!firstDate.isAfter(lastDate)),
        currentDate = CustomDateUtils.dateOnly(currentDate ?? DateTime.now()),
        initialDate = CustomDateUtils.dateOnly(initialDate ?? selectedDate);
